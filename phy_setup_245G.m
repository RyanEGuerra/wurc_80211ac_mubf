...
%   http://www.apache.org/licenses/LICENSE-2.0
    ...
    PHY_OPTS.CHANNEL_BW = 20;   % {40, 20, 10, 5} NOTE: 40 is not supported on WURC HW
    ...
    % Automatic discovery and configuration of WARPv3 nodes.
    % The first line is not necessary, but good for telling you what you
    % did wrong.
    wl_nodesConfig('validate', 'WARP_autoconfig.conf');
    nodesConfig = wl_nodesConfig('read', 'WARP_autoconfig.conf');
    PHY_OPTS.wl_nodes = wl_initNodes(nodesConfig);
    
%     % Hard-coded initialization of WARPv3 nodes
...
%     end
    ...
    % This is a structure that handles the complexity of priming, sending,
    % and listening for a synchronization trigger. Note that since this
    % trigger is sent over Ethernet, it will be subject to transport
    % delays. Thus, packet timing might be different for each node.
    ...
    % Read Trigger IDs into workspace - this may or may not be needed.
    % REG: I think this is the default trigger configuration of the WARPLab
    % nodes, but that may not always be true and/or I might be wrong about
    % that.
    [T_IN_ETH,T_IN_ENERGY,T_IN_AGCDONE,T_IN_REG,T_IN_D0,T_IN_D1,T_IN_D2,T_IN_D3] = ...
        wl_getTriggerInputIDs(PHY_OPTS.AP_NODES(1));
    [T_OUT_BASEBAND, T_OUT_AGC, T_OUT_D0, T_OUT_D1, T_OUT_D2, T_OUT_D3] = ...
        wl_getTriggerOutputIDs(PHY_OPTS.AP_NODES(1));
    wl_triggerManagerCmd(PHY_OPTS.wl_nodes,'output_config_input_selection',...
        [T_OUT_BASEBAND,T_OUT_AGC,T_OUT_D0, T_OUT_D1, T_OUT_D2], [T_IN_ETH,T_IN_REG]);
    ...
    % For the WURCLab 7.4 1x RF design:
    %      RFA, RFB are built-in 2.4/5 GHz;
    %      RFC is the WURC daughter card.
    %      RFD is nothing.
    % For the WURCLab 7.4 4x RF design:
    %      RFA, RFB, RFC, RFD are WURC daughter cards [Q0..Q3]
    %      Built-in radios are disabled.
    [RFA,RFB,RFC,RFD] = wl_getInterfaceIDs(PHY_OPTS.wl_nodes(1));
    PHY_OPTS.available_radios = [RFA,RFB,RFC,RFD];
    PHY_OPTS.STA_RADIOS = RFA; % REG: modified for 2.4/5 operation
...
%     wl_wsdCmd(PHY_OPTS.wl_nodes, 'send_ser_cmd', sum(PHY_OPTS.available_radios), 'L', 0);
    ...
    for node = 1:1:length(PHY_OPTS.AP_NODES)
        for rad = 1:1:length(PHY_OPTS.AP_RADIOS)
            wl_interfaceCmd(PHY_OPTS.AP_NODES(node), PHY_OPTS.AP_RADIOS(rad),...
                'tx_gains', PHY_OPTS.X2829_TX_BB_GAIN, PHY_OPTS.X2829_TX_RF_GAIN);
            wl_interfaceCmd(PHY_OPTS.AP_NODES(node), PHY_OPTS.AP_RADIOS(rad),...
                'channel', PHY_OPTS.X2829_RF_BAND, PHY_OPTS.X2829_LOGICAL_CHANNEL);
...
%             wl_wsdCmd(PHY_OPTS.AP_NODES(node), 'send_ser_cmd', PHY_OPTS.AP_RADIOS(rad), 'B', PHY_OPTS.CENTER_FREQ_WURC);    % Rx Freq
    ...
    for node = 1:1:length(PHY_OPTS.STA_NODES)
        for rad = 1:1:length(PHY_OPTS.STA_RADIOS)
            wl_interfaceCmd(PHY_OPTS.STA_NODES(node), PHY_OPTS.STA_RADIOS(rad),...
                'tx_gains', PHY_OPTS.X2829_TX_BB_GAIN, PHY_OPTS.X2829_TX_RF_GAIN);
            wl_interfaceCmd(PHY_OPTS.STA_NODES(node), PHY_OPTS.STA_RADIOS(rad),...
                'channel', PHY_OPTS.X2829_RF_BAND, PHY_OPTS.X2829_LOGICAL_CHANNEL);
...
%     end
    ...
    for node = 1:1:length(PHY_OPTS.AP_NODES)
        ...
        for rad = 1:1:length(PHY_OPTS.AP_RADIOS)
            if PHY_OPTS.USE_AGC
                wl_interfaceCmd(PHY_OPTS.AP_NODES(node),PHY_OPTS.AP_RADIOS(rad),'rx_gain_mode','automatic');
%                 wl_wsdCmd(PHY_OPTS.AP_NODES(node), 'agc_reset');
%                 wl_wsdCmd(PHY_OPTS.AP_NODES(node), 'rx_gain_mode', PHY_OPTS.AP_RADIOS(rad), 'automatic');
            else
                wl_interfaceCmd(PHY_OPTS.AP_NODES(node),PHY_OPTS.AP_RADIOS(rad),'rx_gain_mode','manual');
                wl_interfaceCmd(PHY_OPTS.AP_NODES(node), PHY_OPTS.AP_RADIOS(rad),...
                    'rx_gains', PHY_OPTS.X2829_RX_RF_GAIN, PHY_OPTS.X2829_RX_BB_GAIN);
...
%             wl_wsdCmd(PHY_OPTS.AP_NODES(node), 'tx_gains', PHY_OPTS.AP_RADIOS(rad), PHY_OPTS.AP_TX_GAIN(rad),99);
        ...
        % Node parameters
        if PHY_OPTS.USE_AGC
            wl_basebandCmd(PHY_OPTS.AP_NODES(node),'agc_target',PHY_OPTS.AGC_WARP_TARGET);
            wl_basebandCmd(PHY_OPTS.AP_NODES(node),'agc_trig_delay', 500);
            wl_basebandCmd(PHY_OPTS.AP_NODES(node),'agc_dco', true);
...
%             wl_wsdCmd(PHY_OPTS.AP_NODES(node), 'agc_reset');
    ...
    for node = 1:1:length(PHY_OPTS.STA_NODES)
        ...
        for rad = 1:1:length(PHY_OPTS.STA_RADIOS)
            if PHY_OPTS.USE_AGC
                wl_interfaceCmd(PHY_OPTS.STA_NODES(node),PHY_OPTS.STA_RADIOS(rad),'rx_gain_mode','automatic');
...
%                 wl_wsdCmd(PHY_OPTS.STA_NODES(node), 'rx_gain_mode', PHY_OPTS.STA_RADIOS(rad), 'automatic');
                ...
                wl_interfaceCmd(PHY_OPTS.STA_NODES(node),PHY_OPTS.STA_RADIOS(rad),'rx_gain_mode','manual');
                wl_interfaceCmd(PHY_OPTS.STA_NODES(node), PHY_OPTS.STA_RADIOS(rad),...
                    'rx_gains', PHY_OPTS.X2829_RX_RF_GAIN, PHY_OPTS.X2829_RX_BB_GAIN);
...
%             wl_wsdCmd(PHY_OPTS.STA_NODES(node), 'tx_gains', PHY_OPTS.STA_RADIOS(rad), PHY_OPTS.STA_TX_GAIN(rad),99);
        ...
        % Node parameters
        if PHY_OPTS.USE_AGC
            wl_basebandCmd(PHY_OPTS.STA_NODES(node),'agc_target',PHY_OPTS.AGC_WARP_TARGET);
            wl_basebandCmd(PHY_OPTS.STA_NODES(node),'agc_trig_delay', 500);
            wl_basebandCmd(PHY_OPTS.STA_NODES(node),'agc_dco', true);
